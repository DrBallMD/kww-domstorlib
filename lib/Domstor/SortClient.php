...
class Domstor_SortClient
{
	...
	{
		if( $this->_type == self::TYPE_PREFIX ) return $this->_fromSourcePrefix();
		if( $this->_type == self::TYPE_ARRAY ) return $this->_fromSourceArray();
	}
	...
	public function getRequestString(array $input = array())
	{
		...
		else
		{
			...
			if( !count($data) ) $data = $this->_default;
		}
		
		$out = '';
		foreach($data as $key => $value)
		{
			if( $this->_type == self::TYPE_PREFIX ) $out.= $this->_methodPrefix($key, $value);
			elseif( $this->_type == self::TYPE_ARRAY ) $out.= $this->_methodArray($key, $value);
		}
		//$out = substr($out, 0, -1);
		return $out;
	}
