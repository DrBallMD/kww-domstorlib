...
class HtmlTable
{
	...
	public function getHtml()
	{
		...
		{
			if( !empty($this->css_class) ) $css=' class="'.$this->css_class.'"';
			$out.="<table$css>";
			if( $this->show_head )
			{
				$out.='<thead>';
...
class HtmlTableField
{
	...
	{
		if( !empty($this->link['css_class']) ) $css=' class="'.$this->link['css_class'].'"';
		if( $this->getValue() ) return '<a'.$css.' href="'.$this->link['href'].'">'.$this->getValue().'</a>';
	}
	...
	public function getHtml()
	{
		...
		if( !empty($css_class) ) $css=' class="'.$css_class.'"';
		if( !empty($this->row_span) ) $span=' rowspan="'.$this->row_span.'"';
	...
	{
		$css = '';
        if( !empty($this->css_class) ) $css=' class="'.$this->css_class.'_head"';
...
class HtmlMinMaxTableField extends HtmlTableField
{
	...
	public function getValue()
	{
        ...
        $out = '';
		if( isset($this->min) ) $this->min_value=$row[$this->min];
		if( isset($this->max) ) $this->max_value=$row[$this->max];
		if( $this->if_equal_show_one and $this->min_value==$this->max_value )
		{
			if( $this->dont_show_if!==$this->min_value ) $out=$this->min_value;
		}
		else
		{
		...
		if( $out and $this->adds) $out.=$this->adds;
		return $out;
	}
}
...
class HtmlLinkedTableField extends HtmlTableField
{
	...
	public function getLink()
	{
		...
		if( !is_array($this->datas) ) $this->datas=array($this->datas);
		$row=$this->table->getRow();
...
class HtmlDelimitedTableField extends HtmlTableField
{
	...
	public function getValue()
	{
		...
		{
			if( $row[$param] !== $this->dont_show_if ) $out.= $row[$param].$this->delimiter;
		}
