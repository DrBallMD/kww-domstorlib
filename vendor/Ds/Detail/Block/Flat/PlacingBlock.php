...
class Ds_Detail_Block_Flat_PlacingBlock extends Ds_Detail_Block_AbstractBlock
{
    ...
    public function windowsDirection()
    {
        ...
        $out = '';
        $data = $this->getData();
        if ($data->get('window_to_south'))
            $out.= 'Юг, ';
        if ($data->get('window_to_north'))
            $out.= 'Север, ';
        if ($data->get('window_to_east'))
            $out.= 'Восток, ';
        if ($data->get('window_to_west'))
            $out.= 'Запад, ';
        $out = substr($out, 0, -2);
        $this->windows = $out;
        return $out;
    ...
    public function parking()
    {
        ...
        if ($data->get('sale_with_parking'))
            $out.= $space . 'Возможна продажа совместно с гаражом или паркоместом';
